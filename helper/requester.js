...
var Requester = {
    ...
    createXHRProxyObject: function () {            
        ...
        xhr.responseCallback = function (handler, data) {
            ...
            // 当后端验证失败时
            if ( String(data.success).replace(/\s/ig,'').toLowerCase() !== 'true' && Requester.backendError && xhr.eventHandlers['action']) {
...
Requester.backendError = function (xhr, data) {
    if (window.bui && Requester && Requester.getByFormName){
        var key, 
            input, 
            formMap = {},
            errorMap = data.field || {}; 
        ...
        for (key in errorMap) { 
            input = Requester.getByFormName(key, xhr.eventHandlers['action']); 
            if (input) { 
                if (bui.Validator) {
                    input.hideError(); 
...
Requester.creatProxyCallback = function(){
    return function(data) {
        ...
        if (data && proxy.action && String(data.success).replace(/\s/ig,'').toLowerCase() !== 'true' ) { 
            if (window.Requester.backendError) {
                window.Requester.backendError(data);
            }
        } 
